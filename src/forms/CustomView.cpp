...
void CustomView::mousePressEvent(QMouseEvent* event)
{
	if (event->button() == Qt::LeftButton && mainWindow->vertex_id != -1)
	{
		...
		RemoveElements::deleteVertice(mainWindow->models[mainWindow->selected_model].triangles[mainWindow->triangle_id].get_vertex_indices()[mainWindow->vertex_id], mainWindow->models[mainWindow->selected_model]);
