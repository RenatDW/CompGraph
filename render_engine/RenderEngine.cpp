     ...
     const int nPolygons = mesh.polygons.size();
        ...
        for (int polygonInd = 0; polygonInd < nPolygons; ++polygonInd) {
            ...
            for (int vertexInPolygonInd = 1; vertexInPolygonInd < nVerticesInPolygon; ++vertexInPolygonInd) {
                painter.drawLine(
                        resultPoints[vertexInPolygonInd - 1].x,
                        resultPoints[vertexInPolygonInd - 1].y,
                        resultPoints[vertexInPolygonInd].x,
                        resultPoints[vertexInPolygonInd].y);
            }

            if (nVerticesInPolygon > 0)
                painter.drawLine(
                        resultPoints[nVerticesInPolygon - 1].x,
                        resultPoints[nVerticesInPolygon - 1].y,
                        resultPoints[0].x,
                        resultPoints[0].y);
        }
    }
