    ...
    if (std::ifstream in(fileContent); in.is_open())
    {
        ...
        while (std::getline(in, line))
        {
            ...
            if (token == OBJ_VERTEX_TOKEN) {
                ...
                result.textureVertices.emplace_back(parseTextureVertex(words_in_line, line_ind));
...
Vector2D ObjReader::parseTextureVertex(const std::vector<std::string> &words_in_line_without_token, const int& line_ind) {
    try {
        return {
        std::stof(words_in_line_without_token.at(0)),
        std::stof(words_in_line_without_token.at(1))};
...
Vector3D ObjReader::parse_normal(const std::vector<std::string> &words_in_line_without_words_in_line_without_token, const int& line_ind) {
    try {
        return {
...
Polygon ObjReader::parse_face(const std::vector<std::string> &words_in_line_without_token, const int& line_ind) {
    ...
    for (const std::string& s : words_in_line_without_token) {
        parseFaceWord(s, one_polygon_vertex_indices, one_polygon_texture_vertex_indices, one_polygon_normal_indices, line_ind);
    }
...
void ObjReader::parseFaceWord(const std::string& word_in_line, std::vector<int> &one_polygon_vertex_indices,
                              std::vector<int> &one_polygon_texture_vertex_indices, std::vector<int> &one_polygon_normal_indices, const int& line_ind) {
    try {
